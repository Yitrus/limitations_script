...
function func_main() {
    ...
    # check dram size 这段检查的作用有限，只是看程序需要的内存会不会超过物理机能提供的
	...
	${TIME} -f "execution time %e (s)" \
	    ${PINNING} numactl --membind=0,2 ${DIR}/bin/launch_bench_nopid ${BENCH_RUN} 2>&1 \
	    | tee ${LOG_DIR}/output.log
    elif [[ "x${BENCH_NAME}" =~ "xspeccpu" ]]; then
	${TIME} -f "execution time %e (s)" \
	    ${PINNING} numactl --membind=0,2 ${DIR}/bin/launch_bench_nopid ${BENCH_RUN} < ${BENCH_ARG} 2>&1 \
	    | tee ${LOG_DIR}/output.log
    else 
	${TIME} -f "execution time %e (s)" \
	    ${PINNING} numactl --membind=0,2 ${DIR}/bin/launch_bench ${BENCH_RUN} 2>&1 \
	    | tee ${LOG_DIR}/output.log
    fi
